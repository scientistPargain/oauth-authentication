...
> {
  ...
  async fetchUser(code: string, state: string, cookies: Pick<Cookies, "get">) {
    ...
    let parsedUserInfo = this.userInfo.parser(userData);

    if (this.urls.userEmail && this.userEmails) {
      const emails = await fetch(this.urls.userEmail, {
        headers: {
          Authorization: `${tokenType} ${accessToken}`,
        },
      })
      ...
      const { emails: fetchedEmails } = emails
        ? this.userEmails.parser(emails)
        : { emails: [] };
      if (!parsedUserInfo.email && fetchedEmails.length > 0) {
        parsedUserInfo = { ...parsedUserInfo, email: fetchedEmails[0].email };
      }
    }
